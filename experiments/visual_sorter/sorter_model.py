...
class ContinuousThoughtMachine(nn.Module):
    ...
    def __init__(self,
        ...
        # --- Output Procesing ---
        self.output_projector = nn.Sequential(nn.LazyLinear(2))
    ...
    def forward(self, x, track=False):
        ...
        predictions = torch.empty(B, 2, self.iterations, device=device, dtype=torch.float32)
        ...
        for stepi in range(self.iterations):
            ...
            current_prediction = self.output_activation(current_prediction)
            current_certainty = torch.zeros(B, 2, device=device, dtype=torch.float32)
